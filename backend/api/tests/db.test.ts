...
await test("token refresh persists a ROTATED refresh_token, not just the access token", async () => {
  ...
  await db.run(
    "INSERT INTO email_accounts (id, user_id, provider, connected_at, is_demo_data, access_token, refresh_token, token_expires_at) VALUES (?, ?, 'mock', ?, 0, ?, ?, ?)",
  ...
  // returning a genuinely NEW refresh token.
