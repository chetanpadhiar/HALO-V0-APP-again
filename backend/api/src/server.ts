...
const server = createServer(async (req, res) => {
    ...
    if (!provider.isConfigured()) {
      sendJson(req, res, 501, {
        error: {
          ...
          message: "Google connection isn't configured on this deployment yet.",
        },
      });
      return;
    }
  ...
  if (url.pathname === "/emails/scan" && req.method === "POST") {
    ...
    if (Array.isArray(body.emails)) {
      ...
      if (!account) {
        sendJson(req, res, 400, { error: { code: "NO_CONNECTED_MAILBOX", message: "HALO isn't connected to a mailbox yet. Connect Google to scan your real inbox, or use demo data." } });
        return;
      }
      ...
      // Gmail messages both funnel through this one
